...
// 从 gin-contrib 中迁移中间件
var middlewareMigrateCommand = &cobra.Command{
	Use:   "migrate",
	Short: "迁移 gin-contrib 中间件，迁移地址：https://github.com/gin-contrib/[middleware].git",
	RunE: func(command *cobra.Command, args []string) error {
		container := command.GetContainer()
		fmt.Println("迁移一个 Gin 中间件")
		var repo string
		{
			prompt := &survey.Input{
				Message: "请输入中间件名称: ",
		...
		// step3: 删除不必要的文件，比如go.mod,go.sum, .git
		...
		filepath.Walk(repoFolder, func(path string, info os.FileInfo, err error) error {
			...
			if isContain {
				...
				c = bytes.ReplaceAll(c, []byte("github.com/gin-gonic/gin"), []byte("github.com/yefangyong/go-frame/framework/gin"))
...
// providerCreateCommand 创建一个新的中间件
